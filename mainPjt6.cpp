...
int main(int argc, char* argv[]) {
    ...
    bool isGraphiquesMode = (argc == 3 && std::string(argv[2]) == "--graphique");
    ...
    try {
        ...
        if (isConsoleMode) {
   		 ...
   		 // Formater le numéro d'itération avec des zéros de remplissage
  		std::ostringstream oss;
    		oss << outputDir << "/iteration_" << std::setw(3) << std::setfill('0') << iteration << ".txt";
   		std::string iterationFile = oss.str();
            ...
            // Variable pour suivre la grille précédente
            std::vector<std::vector<bool>> previousGrid = game.getGrid();
            ...
            while (window.isOpen()) {
                ...
                while (window.pollEvent(event)) {
                    if (event.type == sf::Event::Closed) {
                        std::cout << "La fenêtre a été fermée manuellement à l'itération " << iterationCount << "." << std::endl;
                        window.close();
                    }
                ...
                if (!isPaused) {
                    ...
                    // Vérifier si la grille est identique à la précédente
                    if (game.getGrid() == previousGrid) {
                        std::cout << "La grille s'est stabilisée à l'itération " << iterationCount << ". Fermeture de la fenêtre..." << std::endl;
                        window.close();
                    }

                    // Copier la grille actuelle dans previousGrid
                    previousGrid = game.getGrid();
                }
            ...
            std::cerr << "Erreur : Mode non reconnu. Utilisez '--console' ou '--graphique'." << std::endl;
            return 1;
        }
